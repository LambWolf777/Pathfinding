""" The config module stores all constants and cross-module variables (unfortunately I did not CAPITALIZE constants...).
It also stores the Node and all buttons classes
"""
import sys
import os
from typing import *
...
# Allows to access module that imported/runs this file
    # caller = _getframe(1)
    # if not caller.f_globals["__module__"] == "TerminalTesting":


# pygame generic

if os.path.basename(sys.modules["__main__"].__file__) == "TerminalTesting.py":
    ...
    def update_sym_rect_neighbors(self):
        ...
        def cycle_node(direction, current):
            ...
            while current.is_sym_rect:
                ...
                current = all_nodes[new_col][new_row]
...
class Button:
    ...
    disabled_butt_color = dark_grey
    disabled_text_color = medium_grey
    ...
    def __init__(self, position: Tuple[int, int], text: AnyStr,
        ...
        self.text = text
        self.text_surface = text_font.render(self.text, True, self.get_colors()[1])      # the color will not update
...
class TextInputButton:       # for text buttons
    ...
    def __init__(self, _dict_: Dict, position: [Union[int, float], Union[int, float]],
        ...
        self.headline = text_font.render(text, True, headline_text_color)
        self.headline_rect = self.headline.get_rect()

        self.input_value = number_font.render(str(self.value), True, self.get_colors()[1])
    ...
    def get_surface(self, value: Optional[Union[str, int]] = None) -> Tuple[pg.Surface, Tuple[Union[int, float], Union[int, float]]]:
        """ Return necessary information to append to dirty_blits
        ...
        surface = pg.surface.Surface((self.rect.width, self.rect.height), pg.SRCALPHA)
        surface.fill(light_grey, self.headline_rect)
...
class Checkbox:
    ...
    disabled_check_color = medium_grey
    disabled_box_color = dark_grey
    text_color = box_color = black
    check_color = white
    ...
    def __init__(self, headline: str, position: Tuple[Union[int, float], Union[int, float]],
        ...
        self.headline_surface = text_font.render(self.headline, True, black)
    ...
    def get_surface(self) -> Tuple[pg.Surface, Tuple[Union[int, float], Union[int, float]]]:
        ...
        self.headline_surface = text_font.render(self.headline, True, text_color)
...
class DropDownButton:
    ...
    disabled_butt_color = dark_grey
    disabled_text_color = medium_grey
    disabled_header_color = dark_grey
    ...
    def get_surface(self) -> Tuple[pg.Surface, Tuple[Union[int, float], Union[int, float]]]:
        ...
        self.text_surface = text_font.render(self.text, True, header_color, light_grey)
...
class Stat:
    ...
    def get_surface(self, value: Union[float, int]) -> Tuple[pg.Surface, Tuple[Union[int, float], Union[int, float]]]:
        ...
        text_surface = text_font.render(self.header, True, self.color)
        value_surface = number_font.render(str(value), True, self.color)
