...
# get neighbors moves, so it doesn't load it every time... mayhaps (yes got around 0.5s / 7s)
# moves = {"right": lambda col, row: (col + 1, row, 1),
#          "left": lambda col, row: (col - 1, row, 1),
#          "down": lambda col, row: (col, row + 1, 1),
#          "top": lambda col, row: (col, row - 1, 1),
#          "topright": lambda col, row: (col + 1, row - 1, 1.1421),
#          "topleft": lambda col, row: (col - 1, row - 1, 1.41421),
#          "downright": lambda col, row: (col + 1, row + 1, 1.41421),
#          "downleft": lambda col, row: (col - 1, row + 1, 1.41421)
#          }
...
def select_grid():
    ...
    # Now i cant get it to open with the focus on the file explorer...
    root2 = tkinter.Tk()

    # This allows to make the window completely transparent,
    # .withdraw() was good too but it did not allow me to reopen the file dialog in a while loop...
...
def set_all_neighbors(grid_):
    ...
    # This was slower than what is running now, worth a shot though

    # if diago_allowed:
    #     base_moves = ["right", "left", "down", "top", "topright", "topleft", "downright", "downleft"]
    # else:
    #     base_moves = ["right", "left", "down", "top"]
    #
    # n_col = len(grid)
    # n_row = len(grid[0])

    # This got us modest gains, (around .3s / 2s, almost one second over 8s
    # removing the need to check if col and row == 0 for every node... will keep the try/except in method as it does
    # not seem to require much processing, but it could be handling here

    # could preset the new lists instead of makin copies...

    # for column_no in range(n_col):
    #     if column_no == 0:
    #         moves_ = [move for move in base_moves.copy() if "left" not in move]
    #     else: moves_ = base_moves
    #
    #     print(f"Getting neighbors ... {round(column_no*100/n_col, 1)}%")
    #
    #     for row_no in range(n_row):
    #         if row_no == 0:
    #             final_moves = [move for move in moves_.copy() if "top" not in move]
    #         else: final_moves = moves_
    #
    #         # This is the big chunk of data getting and writing, unable to see it in profiler unless I wrap it in a func
    #         neighbors = {}
    #         for direction in final_moves:
    #             new_col, new_row, cost = moves[direction](column_no, row_no)
    #             try:
    #                 neighbors[direction] = grid_[new_col][new_row], cost
    #             except IndexError:
    #                 pass
    #
    #         grid_[column_no][row_no].neighbors = neighbors
    #
...
def run(algorithm):
    ...
    def init_search(algorithm_: str = algorithm) -> None:
        ...
        :param algorithm_: chosen algorithm as a string (put cfg.algo as parameter)
        :return: None
        """
    ...
    def dij():
        ...
        while not path_found:
            ...
            for node in to_be_removed:
                if node in frontier:  # dont need it actually, but sometimes i do it seems, lol
                    frontier.remove(node)
            to_be_removed.clear()
            ...
            for diago in diago_:
                ...
                for apply_rsr in apply_rsr_:
                    ...
                    for algo in algos_:
                        ...
                        for n in range(n_cycles):
                            print(f"Running {algo.capitalize()}, diago: {diago}, RSR: {apply_rsr} ({n + 1}/{n_cycles})...")
                            start_timer = time.perf_counter_ns()
                            time_sum[f'{algo}len'] = (len(run(algo)))
                            end_timer = time.perf_counter_ns()
                            time_sum[algo] += end_timer - start_timer
                        ...
                        line = algo, diago, apply_rsr, set_neighbors_time, rsr_time, time_sum[algo]/n_cycles, time_sum[f"{algo}len"]
